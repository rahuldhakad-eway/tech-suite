    ...
    public function bulkShift(StoreBulkShift $request)
    {
        ...
        foreach ($employees as $key => $userId) {
            ...
            foreach ($period as $date) {
                ...
                if ($date->greaterThanOrEqualTo($userData->employeeDetail->joining_date) && !in_array($date->format('Y-m-d'), $holidays) && $officeOpenDays && in_array($date->dayOfWeek, $officeOpenDays)) {
