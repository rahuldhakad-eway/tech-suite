...
class PaymentController extends AccountBaseController
{
    ...
    public function addBulkPayments()
    {
        ...
        $this->pendingPayments = Invoice::select('id', 'status', 'client_id', 'invoice_number', 'total', 'sub_total', 'due_amount')->where(function ($q) {
            $q->where('status', 'unpaid');
        ...
        if (request()->ajax() && ($clientId == 'all' || $clientId != null)) {
            $table = view('payments.ajax.bulk-payments', $this->data)->render();

            return Reply::dataOnly(['status' => 'success', 'table' => $table]);
        }
